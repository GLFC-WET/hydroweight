...
```{r quick-plot, fig.width=6, fig.height=5, message = FALSE}
...
```{r hw-run, message=TRUE, warning = FALSE}
...
```{r hw-plot, fig.width=8, fig.height=7, message = FALSE}
...
```{r attr-numeric-map, fig.width=6, fig.height=5, message = FALSE}
ndvi <- toy_dem
...
```{r attr-numeric-run, message = FALSE}
...
```{r attr-cat-map, fig.width=6, fig.height=5, message = FALSE}
...
```{r attr-polynum-map, fig.width=6, fig.height=5}
...
```{r quick-multi, fig.width=6, fig.height=5, message = FALSE}
## Ensure static output in README
tmap::tmap_mode("plot")
...
-   Basic inverse: $(d + 1)^{-1}$\
-   Steeper decay (local emphasis): $(d + 1)^{-2}$\
-   Flatter decay (broad influence): $(d + 1)^{-0.5}$\
-   Scaled inverse (controls effective range): $(c \cdot d + 1)^{-1}$
    with $c = 0.0005$ vs $0.001$\
-   Exponential decay: $\exp(-k d)$ (here $k = 0.002$)\
-   Log‑based inverse: $1 / \log(d_{\mathrm{km}} + 2)$

Distances on the x‑axis are in metres. For functions that use
kilometres, we convert internally as shown.

```{r inv-formulas-ggplot, message=FALSE, warning=FALSE}
...
-   **Broad influence** → smaller exponent or log‑based\
    *Use when distant parts of a watershed retain meaningful
    influence.*\
-   **Adjust range** → scale constant $c$ in $(c \cdot d + 1)^{-1}$\
    *Lower* $c$ flattens decay (longer effective range); higher $c$
    steepens decay.
...
```{r iflo-catchment, fig.width=10, fig.height=3.8}
