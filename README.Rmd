...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.4728558tps://doi.org/10.5281/zenodo.4728558)
...
```{r hw-run, message=TRUE, warning = TRUE}
...
```{r, warning= TRUE, error=FALSE, message=FALSE}
...
2.  For each threshold:
    a.  Re-extract the stream raster with `wbt_extract_streams()`
    b.  Run `hydroweight()` using that stream raster as `target_S`
    c.  Run `hydroweight_attributes()` for the numeric `loi` (NDVI)
    d.  Run `hydroweight_attributes()` for the categorical `loi` (LULC)
...
-   runs `hydroweight_attributes()` twice — once for NDVI (numeric),
    once for LULC (categorical), and
...
```{r threshold-sweep, results='hide'}
...
-   `lumped` and `iEucO` use no stream information – flat lines are
    expected.
-   `iFLO` / `HAiFLO` are flow-path schemes referenced to `target_O`
    (the lake), not `target_S`, so they also change very little with
    threshold.
...
-   Proportions across all classes sum to \~1 within each scheme ×
    ...
    `remove_region`).
-   Stream-referenced schemes (`iEucS`, `iFLS`, `HAiFLS`) are expected
    to show the most variation: as threshold rises and the network
    thins, weights shift spatially, which can tip the proportion balance
    between classes that are near vs. far from stream channels.
