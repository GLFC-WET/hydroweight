...
cover weighted by flow distance to a point). 
...
produces one instance of the `loi` x `distance_weight` summary statistics (i.e., one `loi`, one `roi`, and one `distance_weight`). 
...
`Whitebox` runs `whitebox_tools.exe` which is installed to `your-libary-path/whitebox/WBT`. 
...
We begin by bringing in our toy digital elevation model and use it to generate terrain products.
...
Here we will generate a few targets. Note that the user could provide their 
own vector or raster type targets (see `?hydroweight`). Targets are often 
called *pour points*; here, targets can be a group of raster cells, polygons, polylines, or points.

Below, our first target is a low lying area we will call a lake (`tg_O`). 
All cells <220 m elevation are `TRUE` or `1` and those >220 m are assigned `NA`. 
...
## hwa_test_numeric$return_products$lumped$distance_weight_bounded # Processed distance-weighted raster used in calculating lumped attribute statistics
## ...
## ...
## ...
## hwa_test_numeric$return_products$HAiFLS$distance_weight_bounded # Processed distance-weighted raster used in calculating HAiFLS attribute statistics
...
plot(hwa_test_numeric$return_products$lumped$distance_weight_bounded, axes = F, legend = F, box = FALSE, col = "yellow", add = TRUE)
plot(st_as_sfc(st_bbox(tg_O_catchment)), border = "white", main = "Lumped - distance_weight * ndvi")
plot(hwa_test_numeric$return_products$lumped$`loi_Raster*_bounded` * hwa_test_numeric$return_products$lumped$distance_weight_bounded, axes = F, legend = F, box = FALSE, col = viridis(101), add = TRUE)

## iEucO
plot(st_as_sfc(st_bbox(tg_O_catchment)), border = "white", main = "iEucO - distance_weight")
plot(hwa_test_numeric$return_products$iEucO$distance_weight_bounded, axes = F, legend = F, box = FALSE, col = viridis(101), add = TRUE)
plot(st_as_sfc(st_bbox(tg_O_catchment)), border = "white", main = "iEucO - distance_weight * ndvi")
plot(hwa_test_numeric$return_products$iEucO$`loi_Raster*_bounded` * hwa_test_numeric$return_products$iEucO$distance_weight_bounded, main = "Lumped \n- loi * distance_weight", axes = F, legend = F, box = FALSE, col = viridis(101), add = TRUE)

## iFLS
plot(st_as_sfc(st_bbox(tg_O_catchment)), border = "white", main = "iFLS - distance_weight")
plot(hwa_test_numeric$return_products$iFLS$distance_weight_bounded, axes = F, legend = F, box = FALSE, col = viridis(101), add = TRUE)
plot(st_as_sfc(st_bbox(tg_O_catchment)), border = "white", main = "iFLS - distance_weight * ndvi")
plot(hwa_test_numeric$return_products$iFLS$`loi_Raster*_bounded` * hwa_test_numeric$return_products$iFLS$distance_weight_bounded, main = "Lumped \n- loi * distance_weight", axes = F, legend = F, box = FALSE, col = viridis(101), add = TRUE)
```
...
plot(hwa_test_categorical$return_products$lumped$distance_weight_bounded,
...
plot(hwa_test_categorical$return_products$iEucO$distance_weight_bounded,
  axes = F, legend = F, box = FALSE, col = "yellow", add = TRUE
)
...
plot(hwa_test_categorical$return_products$iFLO$distance_weight_bounded,
  axes = F, legend = F, box = FALSE, col = "yellow", add = TRUE
)
